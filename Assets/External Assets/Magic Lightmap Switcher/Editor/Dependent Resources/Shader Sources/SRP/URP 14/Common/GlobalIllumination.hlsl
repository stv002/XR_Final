...
half3 CalculateIrradianceFromReflectionProbes(half3 reflectVector, float3 positionWS, half perceptualRoughness, float2 normalizedScreenSpaceUV)
{
    ...
    if (weightProbe0 > 0.01f)
    {
        ...
        half4 encodedIrradiance = lerp(
            half4(SAMPLE_TEXTURECUBE_LOD(unity_SpecCube0, samplerunity_SpecCube0, reflectVector0, mip)),
            half4(BlendTwoCubeTextures(0, reflectVector0, mip)),
...
#endif // _REFLECTION_PROBE_BOX_PROJECTION
        half4 encodedIrradiance = lerp(
            half4(SAMPLE_TEXTURECUBE_LOD(unity_SpecCube1, samplerunity_SpecCube1, reflectVector1, mip)),
            half4(BlendTwoCubeTextures(1, reflectVector1, mip)),
            _MLS_ENABLE_REFLECTIONS_BLENDING);
    ...
    {
        half4 encodedIrradiance = lerp(
            half4(SAMPLE_TEXTURECUBE_LOD(_GlossyEnvironmentCubeMap, sampler_GlossyEnvironmentCubeMap, reflectVector, mip)),
            half4(BlendTwoCubeTextures(2, reflectVector, mip)),
            _MLS_ENABLE_REFLECTIONS_BLENDING);
