...
namespace MagicLightmapSwitcher
{
    public class MLSManager : EditorWindow
    {
        ...
        {
            StoredLightingScenario scenarioToRemove = activeSwitcherInstance.availableScenarios[removeIndex];

            if (activeSwitcherInstance == null)
            {
                activeSwitcherInstance = RuntimeAPI.GetSwitcherInstanceStatic(scenarioToRemove.targetScene);
            }

            if (activeSwitcherInstance.workflow == MagicLightmapSwitcher.Workflow.MultiScene)
            {
                activeSwitcherInstance.storedLightmapScenarios.Remove(scenarioToRemove.prefix);
            }
            else
            {
                activeSwitcherInstance.sceneLightmapScenarios.Remove(scenarioToRemove);
            }
            ...
            AssetDatabase.DeleteAsset(AssetDatabase.GetAssetPath(scenarioToRemove));
        }
                ...
                case 0:
                    ...
                    using (new GUILayout.VerticalScope(GUI.skin.box))
                    {
                                    ...
                                    else
                                    {
                                        ...
                                        else
                                        {
                                            ...
                                            if (GUILayout.Button("Store Baked Lightmap", GUILayout.MinWidth(150)))
                                            {
                                                ...
                                                if (activeSwitcherInstance.sceneLightmapDatas != null && activeSwitcherInstance.sceneLightmapDatas.Find(item => item.dataName == lightmapName))
                                                {
                        ...
                        using (new GUILayout.VerticalScope(GUI.skin.box))
                        {
                            ...
                            if (activeSwitcherInstance.storedDataUpdatingProcess)
                            {
                                if (GUILayout.Button("Try To Reload Stored Data"))
                                {
                                    activeSwitcherInstance.storedDataUpdatingProcess = false;
                                    activeSwitcherInstance.StartCoroutine(
                                        activeSwitcherInstance.UpdateStoredArray(
                                            SceneManager.GetSceneAt(selectedScene).name, true));
                                }
                            }
                        }
                    }
        ...
        private static void DrawNoInstanceWarning()
        {
            ...
            {
                MLSLightmapDataStoring.bakingLightingData = false;
                MLSLightmapDataStoring.bakingLightingDataQueue = false;
        ...
        private static void DrawDebugSection(MagicLightmapSwitcher activeInstance)
        {
            if (MLSLightmapDataStoring.bakingLightingData)
            {
                GUI.enabled = false;
        ...
        class AssetsControl : AssetPostprocessor
        {
            static void OnPostprocessAllAssets(string[] importedAssets, string[] deletedAssets, string[] movedAssets, string[] movedFromAssetPaths)
            {
                ...
                if (activeSwitcherInstance == null)
                {
                    RuntimeAPI.GetSwitcherInstanceStatic(SceneManager.GetSceneAt(selectedScene).name);
                }

                if (activeSwitcherInstance != null && !MLSLightmapDataStoring.bakingLightingDataQueue)
                {
                    activeSwitcherInstance.StartCoroutine(
                        activeSwitcherInstance.UpdateStoredArray(SceneManager.GetSceneAt(selectedScene).name, true));
                }
            }
