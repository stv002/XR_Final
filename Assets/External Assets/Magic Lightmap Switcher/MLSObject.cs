...
namespace MagicLightmapSwitcher
{
    public class MLSObject : MonoBehaviour
    {
        ...
        public void Update()
        {
            ...
            if (!switcherInstance.storedDataUpdated || switcherInstance.runtimeAPI == null)
            {
                return;
            }

            if (!switcherInstance.systemProperties.useSwitchingOnly && !switcherInstance.lightingDataSwitching)
            {
                if (meshRenderer != null && meshRenderer.isVisible)
                {
                    if (!switcherInstance.blendingProcess && isSkipped)
                    {
                        Blending.Blend(
                            switcherInstance,
                            switcherInstance.currentLightmapScenario.globalBlendFactor,
                            switcherInstance.currentLightmapScenario,
                            switcherInstance.currentLightmapScenario.targetScene,
                            true);
