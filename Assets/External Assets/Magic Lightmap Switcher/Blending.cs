...
namespace MagicLightmapSwitcher
{
    public class Blending
    {
        ...
        private static List<MagicLightmapSwitcher.AffectedObject> _resultStaticAffectedObjects;
        private static List<MagicLightmapSwitcher.AffectedObject> _resultDynamicAffectedObjects;
        private static List<MLSLight> _resultAffectedLights;
        ...
        public static void Blend(MagicLightmapSwitcher switcherInstance, float blendFactor, StoredLightingScenario storedLightmapScenario, string targetScene, bool forcibly = false)
        {
            ...
            {
                switcherInstance.staticAffectedObjects.TryGetValue(targetScene, out _resultStaticAffectedObjects);
                switcherInstance.dynamicAffectedObjects.TryGetValue(targetScene, out _resultDynamicAffectedObjects);
                switcherInstance.storedLights.TryGetValue(targetScene, out _resultAffectedLights);
            }
            else
            {
                _resultStaticAffectedObjects = switcherInstance.sceneStaticAffectedObjects;
                _resultDynamicAffectedObjects = switcherInstance.sceneDynamicAffectedObjects;
                _resultAffectedLights = switcherInstance.sceneAffectedLightSources;
            }
            ...
            BlendLightmapsData(switcherInstance, storedLightmapScenario, forcibly);
        ...
        )
        {
            ...
            else
            {
                ...
                for (int i = 0; i < _resultDynamicAffectedObjects.Count; i++)
                {
                    if (forcibly && !_resultDynamicAffectedObjects[i].isSkipped)
                    {
                        continue;
                    }
                    
                    if (!_resultDynamicAffectedObjects[i].meshRenderer.isVisible)
                    {
                        _resultDynamicAffectedObjects[i].isSkipped = true;
                        _resultDynamicAffectedObjects[i].mlsObject.isSkipped = true;
                        continue;
                    }

                    _resultDynamicAffectedObjects[i].isSkipped = false;
                    _resultDynamicAffectedObjects[i].mlsObject.isSkipped = false;

                    if (_resultDynamicAffectedObjects[i].meshRenderer != null)
                    {
                         ...
                         switcherInstance.lastLightmapScenario != switcherInstance.currentLightmapScenario))
                    {
                        if ((storedLightingScenario.blendingModules & (1 << 1)) > 0)
                        {
                            ProcessReflectionProbes(
                                _resultDynamicAffectedObjects[i].meshRenderer.reflectionProbeUsage,
                                _resultDynamicAffectedObjects[i],
                                storedLightingScenario.blendableLightmaps,
                                storedLightingScenario.lightingDataFromIndex,
                                storedLightingScenario.lightingDataToIndex);
            ...
            if (switcherInstance.useTexture2DArrays)
            {
                ...
                if ((storedLightingScenario.blendingModules & (1 << 1)) > 0)
                {
                    ...
                    {
                        if (_resultStaticAffectedObjects[i].meshRenderer != null ||
                        ...
                        {
                            if (forcibly && !_resultStaticAffectedObjects[i].isSkipped)
                            {
                                continue;
                            }
                    
                            if (!_resultStaticAffectedObjects[i].meshRenderer.isVisible)
                            {
                                _resultStaticAffectedObjects[i].isSkipped = true;
                                _resultStaticAffectedObjects[i].mlsObject.isSkipped = true;
                                continue;
                            }

                            _resultStaticAffectedObjects[i].isSkipped = false;
                            _resultStaticAffectedObjects[i].mlsObject.isSkipped = false;
                            ...
                            {
                                ProcessReflectionProbes(
                                    _resultStaticAffectedObjects[i].meshRenderer.reflectionProbeUsage,
                        ...
                        {
                            if (_resultStaticAffectedObjects[i].terrain.isActiveAndEnabled)
                            {
                                if (!isDeferredMode)
                                {
                                    ...
                                    {
                                        ProcessReflectionProbes(
                                            _resultStaticAffectedObjects[i].terrain.reflectionProbeUsage,
                                            _resultStaticAffectedObjects[i],
                                            storedLightingScenario.blendableLightmaps,
                                            storedLightingScenario.lightingDataFromIndex,
                                            storedLightingScenario.lightingDataToIndex);
                            ...
                            switcherInstance.lastLightmapScenario != switcherInstance.currentLightmapScenario)
                        {
                            ...
                            if ((storedLightingScenario.blendingModules & (1 << 0)) > 0)
                            {
                                ...
                                if (rendererData == null)
                                {
                                    ...
                                    Debug.LogWarningFormat("<color=cyan>MLS:</color> " +
                                                           "The object \"" +
                                                           _resultStaticAffectedObjects[i].meshRenderer.name + "\" " +
                                    ...
                                    if ((storedLightingScenario.blendingModules & (1 << 0)) > 0)
                                    {
                                        StoredLightmapData.TerrainData terrainData =
                                            storedLightingScenario
                                                    .blendableLightmaps[storedLightingScenario.lightingDataFromIndex]
                                                    .lightingData
                                                    .terrainDataDeserialized[_resultStaticAffectedObjects[i].objectId] as
        ...
        private static void BlendLightSourcesData(float localBlendFactor, float blendFactor, List<StoredLightingScenario.LightmapData> storedLightmapDatas, int fromIndex, int toIndex)
        {
            for (int i = 0; i < _resultAffectedLights.Count; i++)
            {
                ...
                StoredLightmapData.LightSourceData lightFrom = 
                    storedLightmapDatas[fromIndex].lightingData.lightSourceDataDeserialized[_resultAffectedLights[i].lightGUID] as StoredLightmapData.LightSourceData;
                StoredLightmapData.LightSourceData lightTo = 
                    storedLightmapDatas[toIndex].lightingData.lightSourceDataDeserialized[_resultAffectedLights[i].lightGUID] as StoredLightmapData.LightSourceData;
        ...
        private static void BlendGameObjectsData(float localBlendFactor, float blendFactor, List<StoredLightingScenario.LightmapData> storedLightmapDatas, int fromIndex, int toIndex)
        {
            for (int i = 0; i < _resultStaticAffectedObjects.Count; i++)
            {
                ...
                if (rendererDataFrom == null || rendererDataTo == null)
                {
                    _resultStaticAffectedObjects.RemoveAt(i);
                    Debug.LogWarningFormat("<color=cyan>MLS:</color> " +
                        "The object \"" + _resultStaticAffectedObjects[i].meshRenderer.name + "\" " +
                        ...
                        "Object \"" + _resultStaticAffectedObjects[i].meshRenderer.name + "\" might be getting deactivated by some other script.");
                    return;
                }

                if (rendererDataFrom.position != rendererDataTo.position)
                {
                    _resultStaticAffectedObjects[i].meshRenderer.gameObject.transform.position = Vector3.Lerp(
                ...
                {
                    _resultStaticAffectedObjects[i].meshRenderer.gameObject.transform.rotation = Quaternion.Lerp(
